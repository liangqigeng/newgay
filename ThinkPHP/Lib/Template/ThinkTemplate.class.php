...
class  ThinkTemplate {
    ...
    protected function compiler($tmplContent) {
        ...
        if(C('TMPL_STRIP_SPACE')) {
            ...
            $find     = array("~>\s+<~","~>(\s+\n|\r)~");
            $replace  = array('><','>');
            $tmplContent = preg_replace_callback($find, $replace, $tmplContent);
        }
        // 优化生成的php代码
    ...
    public function parse($content) {
        ...
        // 首先替换literal标签内容
        $content = preg_replace_callback('/'.$begin.'literal'.$end.'(.*?)'.$begin.'\/literal'.$end.'/eis',"\$this->parseLiteral('\\1')",$content);
        ...
        //解析普通模板标签 {tagName}
        $content = preg_replace_callback('/('.$this->config['tmpl_begin'].')(\S.+?)('.$this->config['tmpl_end'].')/eis',"\$this->parseTag('\\2')",$content);
        return $content;
    }
    ...
    protected function parsePhp($content) {
        ...
        if(C('TMPL_DENY_PHP') && false !== strpos($content,'<?php')) {
            ...
            $content = preg_replace_callback('/(<\?(?!php|=|$))/i', '<?php echo \'\\1\'; ?>'."\n", $content );
        }
        return $content;
    }
        ...
        foreach ($tLib->getTags() as $name=>$val){
            ...
            foreach ($tags as $tag){
                ...
                if (!$closeTag){
                    ...
                    for($i=0;$i<$level;$i++) $content=preg_replace_callback($patterns,$replacement,$content);
                }
            }
        }
    }
